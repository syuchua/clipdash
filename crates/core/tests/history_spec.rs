...
fn large_text_is_rejected_by_config() {
    let mut h = History::with_config(HistoryConfig { max_items: 10, max_text_bytes: 5, max_image_bytes: 10, ttl_secs: 0 });
    let before = h.len();
...
fn large_image_is_rejected_by_config() {
    let mut h = History::with_config(HistoryConfig { max_items: 10, max_text_bytes: 100, max_image_bytes: 3, ttl_secs: 0 });
    let before = h.len();
...
fn ttl_prunes_unpinned_but_keeps_pinned() {
    let mut h = History::with_config(HistoryConfig { max_items: 10, max_text_bytes: 100, max_image_bytes: 100, ttl_secs: 1 });
    let ida = h.push(text_item("old"));
    let idb = h.push(text_item("old pinned")); h.pin(idb, true);
    let idc = h.push(text_item("fresh"));
    // make a/b very old, keep c fresh
    let mut items = h.all().to_vec();
    for it in &mut items { if it.id != idc { it.ts_ms = 0; } }
